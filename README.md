...
- The schedulers are BPF programs, and to compile them `clang` is needed.
- To compile schedulers, header files from `libbpf-dev` are needed.
- To register the scheduler into the kernel and to generate `vmlinux.h`, `bpftool` must be installed on the system.
